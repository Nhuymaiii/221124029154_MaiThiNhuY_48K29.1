async function loadExcel() {
    ...
    data.forEach(d => {
        ...
        d["SL"] = +d["SL"] || 0;
        d["Nhóm hàng"] = `[${d["Mã nhóm hàng"]}] ${d["Tên mặt hàng"]}`;
    });

    const groupedData = d3.group(data, d => d["Mã nhóm hàng"]);
    const colorScale = d3.scaleOrdinal(d3.schemeCategory10);
    const categoryColors = new Map();
    [...groupedData.keys()].forEach((key, index) => {
        categoryColors.set(key, colorScale(index));
    });

    const revenueByProduct = d3.rollup(
        data,
        v => ({
            "Thành tiền": d3.sum(v, d => d["Thành tiền"]),
            "SL": d3.sum(v, d => d["SL"]),
            "Mã nhóm hàng": v[0]["Mã nhóm hàng"]
        }),
        d => d["Nhóm hàng"]
    );

    const aggregatedData = Array.from(revenueByProduct, ([nhomHang, values]) => ({
        "Nhóm hàng": nhomHang,
        ...values
    }));
    ...
    svg.selectAll(".bar")
        ...
        .attr("fill", d => categoryColors.get(d["Mã nhóm hàng"]))
        .on("mouseover", (event, d) => {
            tooltip.style("visibility", "visible")
                .html(`<strong>Mặt hàng:</strong> ${d["Nhóm hàng"]}<br>
                       <strong>Mã nhóm hàng:</strong> ${d["Mã nhóm hàng"]}<br>
    ...
    svg.append("g")
        ...
        .call(d3.axisBottom(x).tickFormat(d => `${d3.format(",.0f")(d / 1e6)}M`))
        .selectAll("text")
        .style("font-size", "14px")
        .style("font-family", "Calibri, sans-serif");
